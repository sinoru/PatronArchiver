...
protocol SubscribeStarProviding: PatronServiceProviding {
    static var baseURL: URL { get }
}

extension SubscribeStarProviding {
    static var loginURL: URL { baseURL.appending(path: "login") }
    static var accountCheckURL: URL { baseURL.appending(path: "account/settings") }

    static func isLoggedIn(cookies: [HTTPCookie]) -> Bool {
        guard let host = baseURL.host() else { return false }
        return cookies.contains { cookie in
            cookie.name == "_personalization_id" && cookieMatches(cookie, host: host)
        }
    }

    fileprivate static func cookieMatches(_ cookie: HTTPCookie, host: String) -> Bool {
    ...
    @MainActor func extractMetadata(
        ...
        let createdAt = parseLocalizedDate(
    ...
    fileprivate static func decodeCFEmail(_ encoded: String) -> String? {
    ...
    fileprivate func parseLocalizedDate(
        _ string: String?,
        timeZone: TimeZone?
    ) -> Date? {
