...
콘텐츠가 완전히 로드된 상태를 만드는 것은 **Preloader의 책임**이다. 페이지 덤퍼(MHTML, PDF)는 현재 DOM을 있는 그대로 캡처만 한다. 덤퍼에서 로딩 문제를 해결하려 하지 않는다.
...
- 빠른 스크롤은 lazy load 누락을 유발한다. Preloader는 뷰포트 50%씩 점진적으로 스크롤해야 IntersectionObserver가 정상 트리거된다.
